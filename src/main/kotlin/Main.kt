...
fun main(args: Array<String>) {
    ...
    val keystoreInputStream = FileInputStream(keystorePath)

    try {
        keystore.load(keystoreInputStream, keystorePassword)
    } catch (e: IOException) {
        if (e.cause is UnrecoverableKeyException) {
            eprintln("Couldn't load keystore from disk, wrong password")
            exitProcess(1)
        }
    ...
    val secretKeyEntry: SecretKeyEntry = genSecretKeyEntry(aesKey?.decodeHex(), "AES")
